...
mod tests {
    ...
    fn test_all_hrp_prefixes() {
        ...
        assert!(
            bech32_encode(HRP_PUBLIC_KEY, &data32)
                .unwrap()
                .starts_with("cordelia_pk1")
        );
        assert!(
            bech32_encode(HRP_SECRET_KEY, &data32)
                .unwrap()
                .starts_with("cordelia_sk1")
        );
        assert!(
            bech32_encode(HRP_X25519_PK, &data32)
                .unwrap()
                .starts_with("cordelia_xpk1")
        );
        assert!(
            bech32_encode(HRP_PSK, &data32)
                .unwrap()
                .starts_with("cordelia_psk1")
        );
        assert!(
    ...
    fn test_tv_b1_public_key() {
        ...
        assert_eq!(
            encoded,
            "cordelia_pk16adfsqvzky9t042tlmfujeq88g8wzuhnm2nzxfd0qgdx3ac82ydqlx0asz"
        );
    ...
    fn test_tv_b2_secret_key() {
        ...
        assert_eq!(
            encoded,
            "cordelia_sk1n4smr800l4dxpw5yft6f9mpvc3zyn3tf0vexjxts8wkqx89w0asqcef9k6"
        );
        assert_eq!(encoded.len(), 70);
    }
    ...
    fn test_tv_b3_x25519_key() {
        let xpk = hex::decode("d85e07ec22b0ad881537c2f44d662d1a143cf830c57aca4305d85c7a90f6b62e")
            .unwrap();
        let encoded = bech32_encode(HRP_X25519_PK, &xpk).unwrap();
        assert_eq!(
            encoded,
            "cordelia_xpk1mp0q0mpzkzkcs9fhct6y6e3drg2re7psc4av5sc9mpw84y8kkchqsefgnd"
        );
    ...
    fn test_tv_b4_signature() {
        ...
        assert_eq!(
            encoded,
            "cordelia_sig1u4tyxqxrvzk89yyxutxgqm5z32zgwlc7hrjajaxcw0sx2gjfq924lwyzzkg2xwavcc0rjuqulx6xh5jm7hc9jka7y3j4zs2r3eapqzcjyfx4e"
        );
    ...
    #[test]
    fn test_tv_b5_psk() {
        let psk = hex::decode("a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2")
            .unwrap();
        let encoded = bech32_encode(HRP_PSK, &psk).unwrap();
        assert_eq!(
            encoded,
            "cordelia_psk15xev848976nm3jwsu8e28dx96mnl32dsc8fw8a99kmra360s5xeq6lzazn"
        );
        assert_eq!(encoded.len(), 71);
    }
    ...
    fn test_tv_b6_invalid_checksum() {
        let result =
            bech32_decode("cordelia_pk16adfsqvzky9t042tlmfujeq88g8wzuhnm2nzxfd0qgdx3ac82ydqlx0asq");
    ...
    fn test_tv_b7_unknown_hrp() {
        let result = bech32_decode(
            "cordelia_foo16adfsqvzky9t042tlmfujeq88g8wzuhnm2nzxfd0qgdx3ac82ydqpdgfjj",
        );
        assert!(result.is_err());
    }
}
