        ...
        tokio::select! {
            ...
            _ = sync_interval.tick() => {
                ...
                for target in &hot {
                    ...
                    tokio::spawn(async move {
                        // Batched sync (§4.5): one stream per peer, all channels in a loop.
                        // Phase 0 + per-channel sync all on the same stream.
                        let (mut send, mut recv) = match open_bi(&conn).await {
                            Ok(s) => s,
                            Err(e) => { tracing::debug!(peer = %target, error = %e, "sync open_bi failed"); return; }
                        };

                        // Write protocol byte once for the whole batch
                        ...
                        if sync_channels.is_empty() { let _ = send.finish(); return; }
                        ...
                        for ch_id in &sync_channels {
                            ...
                            if stored_count > 0 {
                                tracing::info!(channel = %ch_id, fetched = fetch_ids.len(), stored = stored_count, "pull-sync channel done");
                                total_stored += stored_count as u64;
                            }
                        }
                        // Close send half to signal end of batch
                        let _ = send.finish();
                        if total_stored > 0 {
    ...
    let first_req = match msg {
        cordelia_network::messages::WireMessage::SyncChannelListRequest(_) => {
            ...
            match cordelia_network::codec::read_frame(recv).await {
                Ok(cordelia_network::messages::WireMessage::SyncRequest(r)) => Some(r),
                _ => None, // Peer may close after Phase 0 (no channels to sync)
            }
        }
        cordelia_network::messages::WireMessage::SyncRequest(r) => Some(r),
        _ => return,
    };

    // Batched sync loop (§4.5): serve multiple channels on one stream.
    // After serving each channel's SyncResponse + optional FetchResponse,
    // read the next frame. If it's another SyncRequest, loop. If EOF, done.
    let mut current_req = match first_req {
        Some(r) => r,
        None => return,
    };
    let mut channels_served = 0u32;
    ...
    loop {
        ...
        if !is_swarm_peer {
            ...
            if is_local {
                ...
                let _ = cordelia_network::codec::write_frame(send, &resp).await;
                // Continue to next channel in batch (don't abort the whole stream)
                match cordelia_network::codec::read_frame(recv).await {
                    Ok(cordelia_network::messages::WireMessage::SyncRequest(next)) => {
                        current_req = next;
                        continue;
                    }
                    _ => break, // EOF or unexpected message
                }
            }
        }
        ...
        // Read next frame: FetchRequest (serve items), SyncRequest (next channel), or EOF (done)
        let next_msg = match cordelia_network::codec::read_frame(recv).await {
            Ok(m) => m,
            Err(_) => break, // EOF or error -- stream done
        };

        match next_msg {
            cordelia_network::messages::WireMessage::FetchRequest(freq) => {
                let fetch_items = {
                    let db = match state.db.lock() {
                        Ok(db) => db,
                        Err(_) => return,
                ...
                // After fetch, read next: SyncRequest (next channel) or EOF (done)
                match cordelia_network::codec::read_frame(recv).await {
                    Ok(cordelia_network::messages::WireMessage::SyncRequest(next)) => {
                        current_req = next;
                        continue; // Next channel in batch
                    }
                    _ => break, // EOF or unexpected -- stream done
                }
            }
            cordelia_network::messages::WireMessage::SyncRequest(next) => {
                // No fetch needed for previous channel, move to next
                current_req = next;
                continue;
            }
            _ => break, // Unexpected message type -- stream done
        }
    }

    tracing::debug!(peer = %peer_id, channels = channels_served, "sync stream complete");
}
